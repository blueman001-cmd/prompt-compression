...
def run(combo):
    ...
    try:
        ...
        dataset, mode, zero_shot_CoT, hop, include_label = combo
        suffix = '.jsonl'
        # base_path = f'output/{dataset}/node_classification_pure_structure/test/'
        base_path = f'output/{dataset}/node_classification/test/'
        ...
        if mode == 'ego':
            path_list = [
                ...
                # '1_hop_without_label',
                # '2_hop_without_label',
                # '2_hop_without_label_st',
                # '2_hop_without_label_se',
                '2_hop_without_label_st_se',
                # '2_hop_without_label_st_se_no_su',
        ...
        for p in path_list:
            ...
            try:
                with open(path, 'r') as file:
                    for line in tqdm(file, desc="Processing lines", unit="_line"):
                        if count > 500:
                            break
                        try:
                        ...
                        try:
                            if zero_shot_CoT:
                                result_dict = get_completion_from_messages(
                            ...
                            sum_input_tokens += result_dict["input_tokens"]
            ...
            avg_input_tokens = sum_input_tokens / count if count > 0 else 0
            ...
            try:
                with open('results/t1.txt', 'a') as f:
                    f.write(
                        f"{path} accuracy: {right}/{count} ({accuracy:.2%})  "
                        f"time:{h}h {m}m {s:.2f}s  "
                        f"input_tokens:{sum_input_tokens}  "
                        f"avg_input_tokens:{avg_input_tokens:.2f}\n"
                    )
            except Exception as e:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset_name1', type=str, required=True, help="Name of the dataset1")
    parser.add_argument('--dataset_name2', type=str, required=True, help="Name of the dataset2")#迁移学习
    ...
    dataset1 = args.dataset_name1
    dataset2 = args.dataset_name2
    model_size = args.model_size
    scenario = args.scenario
    mode = args.mode
    hop = 1 if args.hop == '1 hop' else 2
    ...
    model_name = f'llama_{model_size}_Instruct_{dataset1}_node_2_hop_without_label_st_se_10' #tuned model
    if scenario == 'no tune':
        model_name += '_no_tune'
