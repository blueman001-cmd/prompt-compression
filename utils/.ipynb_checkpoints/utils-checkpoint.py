...
def simple_text_score(target_title, neighbor_title):
    t1 = set(tokenize_title(target_title))
    t2 = set(tokenize_title(neighbor_title))
    if not t1 or not t2:
        return 0.0
    return len(t1 & t2) / len(t1 | t2)

def get_node_degree(node_idx, edge_index):
    return int(((edge_index[0] == node_idx) | (edge_index[1] == node_idx)).sum().item())

def rank_neighbors_by_score(target_idx, neighbors, text, data, alpha=0):
    target_title = text['title'][target_idx]
    scored = []
    degrees = [get_node_degree(n, data.edge_index) for n in neighbors] if len(neighbors) > 0 else [1]
    max_deg = max(degrees) if degrees else 1

    for n in neighbors:
        semantic = simple_text_score(target_title, text['title'][n])
        degree = get_node_degree(n, data.edge_index) / max_deg if max_deg > 0 else 0.0
        score = alpha * semantic + (1 - alpha) * degree
        scored.append((n, score, semantic, degree))

    scored.sort(key=lambda x: x[1], reverse=True)
    return scored

def summarize_titles(node_ids, text, topn=5):
    words = []
    stopwords = {
        "the", "a", "an", "for", "of", "and", "to", "in", "on", "with",
        "using", "based", "from", "by", "via", "study", "analysis","title"
    }
    for nid in node_ids:
        words.extend([w for w in tokenize_title(text['title'][nid]) if w not in stopwords and len(w) > 2])

    if not words:
        return "No clear shared topic."
    common = [w for w, _ in Counter(words).most_common(topn)]
    return ", ".join(common)

def generate_compressed_structure_prompt(
    node_index,
    text,
    all_hops,
    data,
    ...
    topk_1hop=5,
    topk_2hop=2,
    include_2hop_summary=True
):
    prompt_str = ""
    target_word = "Product id" if dataset == "product" else "Paper id"

    one_hop = list(np.unique(np.array(all_hops[0]))) if len(all_hops) > 0 else []
    two_hop = list(np.unique(np.array(all_hops[1]))) if len(all_hops) > 1 else []

    ranked_1hop = rank_neighbors_by_score(node_index, one_hop, text, data)
    ranked_2hop = rank_neighbors_by_score(node_index, two_hop, text, data)

    selected_1hop = [x[0] for x in ranked_1hop[:topk_1hop]]
    selected_2hop = [x[0] for x in ranked_2hop[:topk_2hop]]

    if selected_1hop:
        prompt_str += "\n## Key 1-hop neighbors:\n"
        for n in selected_1hop:
            prompt_str += f"\n{target_word}: {n}\n"
            prompt_str += f"Title: {text['title'][n]}\n"
            if include_label:
                if is_train and data.train_mask[n]:
                    prompt_str += f"Label: {text['label'][n]}\n"
                elif (not is_train) and (data.train_mask[n] or data.val_mask[n]):
                    prompt_str += f"Label: {text['label'][n]}\n"

    if selected_2hop:
        prompt_str += "\n## Key 2-hop neighbors:\n"
        for n in selected_2hop:
            prompt_str += f"\n{target_word}: {n}\n"
            prompt_str += f"Title: {text['title'][n]}\n"
            if include_label:
                if is_train and data.train_mask[n]:
    ...
    if include_2hop_summary and len(two_hop) > 0:
        topic_summary = summarize_titles(two_hop, text, topn=3)
        prompt_str += "\n## 2-hop neighborhood summary:\n"
        prompt_str += f"Main shared topics: {topic_summary}\n"
...
def get_and_save_message_for_node(node_index_list, data, text, dataset, source, save_dir, hop=2,
    ...
    for node_index in tqdm(node_index_list, desc="Processing nodes"):
         ...
         if mode == 'compressed_neighbors':
            ...
            prompt_str = generate_compressed_structure_prompt(
                ...
                topk_1hop=max_papers_1,
                topk_2hop=max_papers_2,
                include_2hop_summary=True
            )
            context += prompt_str
            ...
            output_file = path + f'/{dir}/{hop}_hop_{has_label}_st.jsonl'
...
def get_and_save_message_for_link(node_index_list, data, text, dataset, source, save_dir, max_papers_1=10,
    ...
    for node_index in tqdm(node_index_list, desc="Processing links"):
        ...
        if case == 0:
            ...
            original_neighbors = all_hops[0].copy()
            ...
            while True:
                ...
                if sample_node != node_index and sample_node not in original_neighbors:
